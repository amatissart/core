...
core.reportRequestError = function reportRequestError(err, res) {
  core.reportError((error) => {
    res
      .status(core.isNoTileError(error) ? 404 : 400)
      .header('Cache-Control', 'public, s-maxage=30, max-age=30')
      .json(error.message || 'error/unknown');
    // Any error that has metrics setting does not need to go into the error log
    core.log(error.metrics ? 'info' : 'error', err);
    core.metrics.increment(error.metrics || 'err.unknown');
  }, err);
};
